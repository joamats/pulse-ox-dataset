...
DROP TABLE IF EXISTS `golden-rite-376204.mimiciii_pulseOx.pivoted_lab_extended`;
CREATE TABLE `golden-rite-376204.mimiciii_pulseOx.pivoted_lab_extended` AS
...
SELECT
  ...
  , avg(CASE WHEN label = 'RBC' THEN valuenum ELSE null END) as RBC
  , avg(CASE WHEN label = 'd_dimer' THEN valuenum ELSE null END) as d_dimer
  ...
  , avg(CASE WHEN label = 'fibrinogen' THEN valuenum ELSE null END) as fibrinogen
  , avg(CASE WHEN label = 'GLUCOSE' THEN valuenum ELSE null END) as GLUCOSE
  , avg(CASE WHEN label = 'MCHC' THEN valuenum ELSE null END) as MCHC
  ...
  , avg(CASE WHEN label = 'MCV' THEN valuenum ELSE null END) as MCV
  , avg(CASE WHEN label = 'alt' THEN valuenum ELSE null END) as alt
  , avg(CASE WHEN label = 'alp' THEN valuenum ELSE null END) as alp
  , avg(CASE WHEN label = 'ast' THEN valuenum ELSE null END) as ast
  , avg(CASE WHEN label = 'bilirubin_total' THEN valuenum ELSE null END) as bilirubin_total
  , avg(CASE WHEN label = 'bilirubin_direct' THEN valuenum ELSE null END) as bilirubin_direct
  , avg(CASE WHEN label = 'bilirubin_indirect' THEN valuenum ELSE null END) as bilirubin_indirect
  , avg(CASE WHEN label = 'ck_cpk' THEN valuenum ELSE null END) as ck_cpk
  , avg(CASE WHEN label = 'ck_mb' THEN valuenum ELSE null END) as ck_mb
  , avg(CASE WHEN label = 'ggt' THEN valuenum ELSE null END) as ggt
  , avg(CASE WHEN label = 'ld_ldh' THEN valuenum ELSE null END) as ld_ldh
  ...
  , avg(CASE WHEN label='MCH' THEN valuenum ELSE null end) as MCH
  , avg(CASE WHEN label='thrombin' THEN valuenum ELSE null end) as thrombin
...
( -- begin query that extracts the data
  ...
  , CASE
        ...
        WHEN itemid = 50863 THEN 'alp'
        WHEN itemid = 50878 THEN 'ast'
        WHEN itemid = 50885 THEN 'bilirubin_total'
        WHEN itemid = 50883 THEN 'bilirubin_direct'
        WHEN itemid = 50884 THEN 'bilirubin_indirect'
        WHEN itemid = 50910 THEN 'ck_cpk'
        WHEN itemid = 50911 THEN 'ck_mb'
        WHEN itemid = 50927 THEN 'ggt'
        WHEN itemid = 50954 THEN 'ld_ldh'
        ...
        WHEN itemid = 51297 THEN 'THROMBIN'
      ELSE null
    END AS label
  ...
  -- the where clause below requires all valuenum to be > 0, so these are only upper limit checks
    CASE
      ...
      WHEN itemid = 51248 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50861 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50863 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50878 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50885 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50883 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50884 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50910 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50911 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50927 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 50954 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 51249 and valuenum <     0 THEN null -- can not be null
      WHEN itemid = 51196 and valuenum <     0 THEN null 
      WHEN itemid = 51214 and valuenum <     0 THEN null
      WHEN itemid = 51277 and valuenum <     0 THEN null -- can not be null
      ...
      WHEN itemid = 51279 and valuenum <     0 THEN null -- RBC
      WHEN itemid = 51297 and valuenum <     0 THEN null -- thrombin
      ...
      WHEN itemid = 51250 and valuenum <  0 THEN NULL -- 'MCV'
